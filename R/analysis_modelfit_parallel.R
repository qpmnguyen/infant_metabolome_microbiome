...
option_list <- list(
  ...
  make_option("--parallel", type = "logical", default = FALSE, help = "Determine whether or not reps are ran in parallel")
)
...
modelfit.fn <- function(response, predictors, model, in.folds, out.folds, metabtype,
  ...
  for(i in 1:length(folds)){
    ...
    # fitting models 
    if (model %in% c("enet", "spls", "rf", "svm_rbf", "gbm", "xgboost")){
...
if (opt$parallel == T){
  ...
  # running results as repeated CV with parallel
  results <- foreach(j = 1:opt$nrep) %dopar% {
  ...
  # running results as repeated CV without parallel
  results <- foreach(j = 1:opt$nrep) %do% {
