...
for (i in 1:length(eval)){
  for (j in 1:length(timepoints)){
    ...
    if (file.exists(glue("output/analyses/prediction/processed/{time}_tar_{eval}_bayes.rds", time = timepoints[j], eval = eval[i])) == FALSE){
      mods <- get_bayesian(eval = eval[i], time = timepoints[j], type = "tar")
      saveRDS(tidy(mods), glue("output/analyses/prediction/processed/{time}_tar_{eval}_bayes.rds", time = timepoints[j], eval = eval[i]))
    } else {
      mods <- readRDS(file = glue("output/analyses/prediction/processed/{time}_tar_{eval}_bayes.rds", time = timepoints[j], eval = eval[i]))
    }
