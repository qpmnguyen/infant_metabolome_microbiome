...
def system_info(plat):
    if plat == "Darwin":
        if os.path.exists("/Volumes/rc-1/Lab/") == True:
            ...
            return(["/Volumes/rc/Lab/QNguyen/ResultsFiles/data/raw_{time}_{met}_phyloseq_obj.rds"])
    elif plat == "Linux":
        if platform.node() in ["polaris.dartmouth.edu", "andes.dartmouth.edu"]:
            return(["/dartfs-hpc/rc/lab/H/HoenA/Lab/QNguyen/ResultsFiles/data/raw_{time}_{met}_phyloseq_obj.rds"])
        else:
